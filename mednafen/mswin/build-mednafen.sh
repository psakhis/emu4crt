...
PKG_CONFIG_PATH="$CROSS64_PATH/lib/pkgconfig" PATH="$CROSS64_PATH/bin:$PATH" CPPFLAGS="-I$CROSS64_PATH/include -DUNICODE=1 -D_UNICODE=1" LDFLAGS="-L$CROSS64_PATH/lib -static-libstdc++ -lswitchres" ../../configure --host=x86_64-w64-mingw32 --disable-alsa --disable-jack --enable-threads=win32 --with-sdl-prefix="$CROSS64_PATH" && \
make -j4 V=0 && \
cd .. && \
cd build64
cd src
